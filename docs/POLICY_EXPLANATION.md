...
**Measured:** at settled state, trust is 0.90 for bursty-legit and 0.99 for normal-legit — both high, because both spend the large majority of their time looking clean. Trust for both attacker classes sits at **0.0003 or below** — effectively zero, because neither is ever clean long enough to accrue any. That asymmetry is what lets a bursty-legit client's burst be tolerated (mean 3.1s throttled per isolated burst) while a sustained attacker escalates within seconds.
...
| L3 (trust + persistence) | `LegitimateAdmission`, `LegitimateBlockSafety` | +12.64; trust 0.90-0.99 legit vs. ≤0.0003 attackers |
